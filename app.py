...
def index():
    ...
    if conn:
        ...
        if busqueda:
            # Buscar equipos que coincidan con la búsqueda
            query = """
                SELECT * FROM equipos 
                WHERE codigo LIKE %s 
                   OR tipo_equipo LIKE %s 
                   OR marca LIKE %s 
                   OR modelo LIKE %s 
                   OR sistema_operativo LIKE %s
    ...
    return render_template('index.html', equipos=equipos, hoy=hoy, busqueda=busqueda)

@app.route('/registrar', methods=['POST'])
def registrar():
    """CREATE: Registrar un nuevo equipo"""
    try:
...
def editar(id):
    """READ: Mostrar formulario de edición con datos del equipo"""
    conn = get_db_connection()
    if conn:
        cursor = conn.cursor(dictionary=True)
        ...
        if equipo:
            hoy = datetime.now().strftime('%Y-%m-%d')
            return render_template('editar.html', equipo=equipo, hoy=hoy)
        else:
...
def actualizar(id):
    """UPDATE: Actualizar un equipo existente"""
    try:
        # Obtener datos del formulario
        codigo = request.form['codigo'].strip().upper()
        tipo_equipo = request.form['tipo_equipo']
        ...
        if conn:
            ...
            cursor.execute("SELECT id FROM equipos WHERE codigo = %s AND id != %s", (codigo, id))
            if cursor.fetchone():
                flash(f'El código {codigo} ya existe en otro equipo', 'danger')
                cursor.close()
                conn.close()
                return redirect(f'/editar/{id}')
            ...
            cursor.execute('''
                UPDATE equipos 
                SET codigo = %s, tipo_equipo = %s, marca = %s, modelo = %s, 
                    sistema_operativo = %s, almacenamiento_gb = %s, ram = %s, 
                    estado = %s, fecha_mantenimiento = %s, fecha_registro = %s
                WHERE id = %s
            ''', (codigo, tipo_equipo, marca, modelo, sistema_operativo,
                  almacenamiento_gb, ram, estado, fecha_mantenimiento, fecha_registro, id))
            
            conn.commit()
            cursor.close()
            conn.close()
            
            flash(f'✅ Equipo {codigo} actualizado exitosamente!', 'success')
        else:
            flash('❌ Error de conexión a la base de datos', 'danger')
...
def buscar():
    ...
    return redirect(f'/?busqueda={termino}')
